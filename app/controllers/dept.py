...
class DeptController(CRUDBase[Dept, DeptCreate, DeptUpdate]):
    ...
    async def get_dept_tree(self, name):
        q = Q()
        # Get all departments that have not been soft deleted
        q &= Q(is_deleted=False)
        if name:
            q &= Q(name__contains=name)
        all_depts = await self.model.filter(q).order_by("order")

        # Helper function to recursively build the department tree
        ...
        # Build the department tree starting from the top-level departments (parent_id=0)
