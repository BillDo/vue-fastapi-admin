...
@router.get("/list", summary="View role list")
async def list_role(
    page: int = Query(1, description="Page number"),
    page_size: int = Query(10, description="Items per page"),
    role_name: str = Query("", description="Role name for filtering"),
):
    q = Q()
    if role_name:
...
@router.get("/get", summary="View role")
async def get_role(
...
@router.get("/authorized", summary="View role permissions")
...
@router.post("/authorized", summary="Update role permissions")
